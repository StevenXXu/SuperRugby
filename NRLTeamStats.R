...
NRLteamStats<-data.frame(Team=character(),HomeScores=numeric(),HomeHalfScores=numeric(),HomeScrums=numeric(),
...
for(i in 1:nrow(teams)){
  ...
  NRLteamStats[i,] <- data.frame(team_a,mean(home_data[,4]),mean(home_data[,9]),mean(home_data[,10]),
...
saveRDS(NRLteamStats, "NRLteamStats.rda")
